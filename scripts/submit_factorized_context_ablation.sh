...
exec sbatch \
  --job-name=be-context-ablation \
  --mail-user="${notify_email}" \
  --mail-type=ALL \
  --export=ALL,BE_RESCUE_FEATURE_MODES="context-uncertainty context-text context-question context-answer-uncertainty context",BE_RESCUE_FEATURES="${features}",BE_RESCUE_ROLLOUTS="${rollouts}",BE_RESCUE_OUTPUT_DIR="${output_dir}" \
