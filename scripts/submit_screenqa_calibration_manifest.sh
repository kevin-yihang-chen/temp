...
for candidate_file in model.json selected-ranker-report.json candidate.audit.json SHA256SUMS; do
  if [[ ! -s "${candidate_dir}/${candidate_file}" ]]; then
    echo "frozen ScreenQA candidate file is missing: ${candidate_file}" >&2
    exit 2
  fi
done
if [[ -e "${calibration_dir}" ]]; then
  echo "refusing to reuse ScreenQA calibration manifest: ${calibration_dir}" >&2
  exit 2
fi
...
if [[ -n "$(git -C "${repo_dir}" status --porcelain --untracked-files=no)" ]]; then
  echo "tracked worktree must be clean before ScreenQA calibration submission" >&2
  exit 2
fi
...
cd "${repo_dir}"
PYTHONPATH="${repo_dir}/src" "${python_bin}" - "${candidate_dir}" <<'PY'
import json
import sys
from pathlib import Path

from scripts.export_screenqa_calibration_manifest import verify_candidate

print(json.dumps(verify_candidate(Path(sys.argv[1])), indent=2, sort_keys=True))
PY

candidate_bundle_sha256=$(sha256sum "${candidate_dir}/SHA256SUMS")
candidate_bundle_sha256=${candidate_bundle_sha256%% *}
code_revision=$(git -C "${repo_dir}" rev-parse HEAD)
export_args="ALL,BE_SCREENQA_CANDIDATE_DIR=${candidate_dir},BE_SCREENQA_CANDIDATE_BUNDLE_SHA256=${candidate_bundle_sha256},BE_SCREENQA_CALIBRATION_MANIFEST_DIR=${calibration_dir},BE_SCREENQA_FORMAL_MANIFEST_DIR=${formal_dir},BE_SCREENQA_RESERVE_MANIFEST_DIR=${reserve_dir},BE_SCREENQA_UNTOUCHED_MANIFEST_DIR=${untouched_dir},BE_SCREENQA_EXPECTED_CODE_REVISION=${code_revision}"
...
submission=$(
  sbatch \
    ...
    --mail-type=ALL \
    --export="${export_args}" \
    "${repo_dir}/scripts/slurm_screenqa_calibration_manifest.sh"
)
job_id=${submission##* }
if [[ ! "${job_id}" =~ ^[0-9]+$ ]]; then
  echo "could not parse ScreenQA calibration-export job ID: ${submission}" >&2
  exit 2
fi
printf '%s\n' "${submission}"
printf 'screenqa_calibration_export_job_id=%s code_revision=%s candidate_bundle_sha256=%s\n' \
  "${job_id}" "${code_revision}" "${candidate_bundle_sha256}"
