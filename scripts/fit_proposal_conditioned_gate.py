...
from beyond_entropy.proposal_conditioned_gate import (
    PROPOSAL_CONDITIONED_BOOTSTRAP_RESAMPLES,
    PROPOSAL_CONDITIONED_SEED,
    evaluate_proposal_conditioned_gate,
)
...
def _checked(path: Path, expected: str, name: str) -> Path:
    ...
    if not resolved.is_file() or _sha256(resolved) != expected:
        raise ValueError(f"proposal-conditioned {name} SHA-256 mismatch")
    return resolved
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fit the frozen loss-proposal-conditioned rescue/harm gate"
    )
    for name in (
        "rollouts",
        "features",
    ...
    if output_dir.exists():
        raise FileExistsError(
            f"refusing to overwrite proposal-conditioned output: {output_dir}"
        )
    ...
    report, score_report, model, score_rows = evaluate_proposal_conditioned_gate(
        {"docvqa": records},
        ...
        bootstrap_resamples=PROPOSAL_CONDITIONED_BOOTSTRAP_RESAMPLES,
        seed=PROPOSAL_CONDITIONED_SEED,
    )
