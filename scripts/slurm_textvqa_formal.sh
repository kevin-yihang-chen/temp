...
#SBATCH --output=/userhome/cs3/yihangc/Documents/beyond-entropy/slurm-textvqa-formal-%x-%j.out
...
"${python_bin}" -m beyond_entropy collect-qwen \
  ...
  --scientific-status "outcome-unseen TextVQA formal confirmation; frozen OOF action-value model" \
  --max-new-tokens 32 \
  --min-pixels 200704 \
  --max-pixels 602112 \
