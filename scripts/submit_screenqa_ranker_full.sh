...
shard_count=4
run_root="${repo_dir}/artifacts/screenqa-train-factorized-v1/ranker-rollouts-v1"
...
if [[ -e "${run_root}" ]]; then
  echo "refusing to reuse ScreenQA ranker run root: ${run_root}" >&2
  exit 2
fi
if [[ -n "$(git -C "${repo_dir}" status --porcelain --untracked-files=no)" ]]; then
  echo "tracked worktree must be clean before ScreenQA ranker submission" >&2
  exit 2
fi
...
array_submission=$(
  sbatch \
    ...
    --mail-type=ALL \
    --export="${export_args}" \
    "${repo_dir}/scripts/slurm_screenqa_ranker_full.sh"
)
array_job_id=${array_submission##* }
if [[ ! "${array_job_id}" =~ ^[0-9]+$ ]]; then
  echo "could not parse ScreenQA ranker array job ID: ${array_submission}" >&2
  exit 2
fi
printf '%s\n' "${array_submission}"
printf 'screenqa_ranker_array_job_id=%s code_revision=%s\n' \
...
merge_submission=$(
  sbatch \
    ...
    --mail-type=ALL \
    --export="${export_args}" \
    "${repo_dir}/scripts/slurm_screenqa_ranker_full_merge.sh"
)
merge_job_id=${merge_submission##* }
if [[ ! "${merge_job_id}" =~ ^[0-9]+$ ]]; then
  echo "could not parse ScreenQA ranker merge job ID: ${merge_submission}" >&2
  exit 2
fi
printf '%s\n' "${merge_submission}"
printf 'screenqa_ranker_merge_job_id=%s code_revision=%s\n' \
  "${merge_job_id}" "${code_revision}"
