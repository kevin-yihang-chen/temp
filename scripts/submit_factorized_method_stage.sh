...
stage=${1:?usage: submit_factorized_method_stage.sh smoke|pilot}
case "${stage}" in
  smoke)
    ...
    factorized_config=configs/factorized_method_smoke_v1.json
    evaluation_config=configs/cv_method_smoke_evaluation_v1.json
    output_root=artifacts/factorized-potential-outcomes-v1/phase-a-smoke
    ;;
  pilot)
    ...
    factorized_config=configs/factorized_method_pilot_v1.json
    evaluation_config=configs/cv_method_pilot_evaluation_v1.json
    output_root=artifacts/factorized-potential-outcomes-v1/phase-b-pilot
    ;;
...
sbatch --export="ALL,CV_METHOD_PLAN=${repo_dir}/${plan},CV_METHOD_PLAN_SHA256=${plan_sha}" \
  scripts/slurm_factorized_method_stage.sh
