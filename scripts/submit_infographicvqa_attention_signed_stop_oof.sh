...
if [[ -n "$(git status --porcelain --untracked-files=no)" ]]; then
  echo "tracked worktree must be clean before attention signed stop" >&2
  exit 2
fi
for path in "${worker}" "${runner}" "${module}" "${protocol}" \
  "${attention_root}/merged-features/features-question-region-attention-label-free.pt" \
  "${attention_root}/attention-stop-factorization-v1/complete.json"; do
  if [[ ! -f "${path}" ]]; then
    echo "attention signed stop input is incomplete: ${path}" >&2
    exit 2
  fi
done
if [[ -e "${output_dir}" ]]; then
  echo "attention signed stop output already exists" >&2
  exit 2
fi
...
if [[ -z "${gpu_limit}" || -z "${gpu_used}" || -z "${cpu_limit}" \
  ...
  echo "attention signed stop needs 45 GPU-minutes and 180 CPU-minutes" >&2
  exit 2
fi
...
job_id=${submission%%;*}
if [[ ! "${job_id}" =~ ^[0-9]+$ ]]; then
  echo "could not parse attention signed stop Slurm job ID: ${submission}" >&2
  exit 2
fi
printf 'attention_signed_stop_job_id=%s code_revision=%s evaluator_device=cpu reserved_gpu=rtx_4090\n' \
  "${job_id}" "${revision}"
