...
if [[ ! -f "${worker}" || ! -f "${protocol}" ]]; then
  echo "attention-where worker or protocol is absent" >&2
  exit 2
fi
...
if [[ -z "${gpu_limit}" || -z "${gpu_used}" || $((gpu_limit - gpu_used)) -lt 960 ]]; then
  echo "attention-where needs a 960 GPU-minute reserve" >&2
  exit 2
fi
...
submit_epoch=$(date +%s)
args=("${revision}" "${worker_sha256}" "${protocol_sha256}" "${resume}" "${submit_epoch}")
...
printf 'infographicvqa_attention_where_job_id=%s code_revision=%s gpu_type=h800 gpu_count=4 resume=%s\n' \
