...
#SBATCH --output=/userhome/cs3/yihangc/Documents/beyond-entropy/slurm-textvqa-attention-fresh-formal-%j.out
...
"${python_bin}" -m beyond_entropy collect-qwen \
  ...
  --scientific-status "outcome-unseen fresh-split TextVQA attention action-value confirmation" \
  --max-new-tokens 32 \
  --min-pixels 200704 \
  --max-pixels 602112 \
