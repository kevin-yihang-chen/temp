...
#SBATCH --array=0-3
#SBATCH --job-name=be-screenqa-cal-bank
#SBATCH --output=/userhome/cs3/yihangc/Documents/beyond-entropy/slurm-screenqa-calibration-bank-%A-%a.out
...
shard_dir="${BE_SCREENQA_CALIBRATION_RUN_ROOT}/${shard_name}"
rollouts="${shard_dir}/rollouts.jsonl"

if [[ "${BE_SCREENQA_CALIBRATION_EXPECTED_STATES}" -ne 9951 ]]; then
  echo "ScreenQA calibration collection is frozen to 9951 states" >&2
  exit 2
fi
if [[ "${BE_SCREENQA_SHARD_COUNT}" -ne 4 ]]; then
  echo "ScreenQA calibration collection is frozen to four shards" >&2
  exit 2
fi
if [[ "${shard_index}" -lt 0 || "${shard_index}" -ge "${BE_SCREENQA_SHARD_COUNT}" ]]; then
  echo "ScreenQA calibration shard index is out of range" >&2
  exit 2
fi
tracked_status=$(git -C "${repo_dir}" status --porcelain --untracked-files=no)
if [[ -n "${tracked_status}" ]]; then
  echo "Tracked worktree must be clean before ScreenQA calibration collection" >&2
  exit 2
fi
actual_code_revision=$(git -C "${repo_dir}" rev-parse HEAD)
if [[ "${actual_code_revision}" != "${BE_SCREENQA_EXPECTED_CODE_REVISION}" ]]; then
  echo "ScreenQA calibration collection code revision mismatch" >&2
  exit 2
fi
...
collect_args=(
  ...
  --scientific-status "ScreenQA frozen risk-calibration sibling bank; outcomes may calibrate only the frozen threshold sequence"
  --max-new-tokens 32
  --min-pixels 200704
  --max-pixels 602112
...
if [[ "${first_sha}" != "${second_sha}" ]]; then
  echo "ScreenQA resume changed completed calibration rollout bytes" >&2
  exit 2
fi
...
if not passed:
    raise SystemExit("ScreenQA calibration resume audit failed")
