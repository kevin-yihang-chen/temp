...
output_dir="${repo_dir}/artifacts/gate2-chartqa-2500/context-quadrant-two-stage-nested-oof-v9"
...
exec sbatch \
  --job-name=be-context-quadrant \
  --mail-user="${notify_email}" \
  --mail-type=ALL \
  --export=ALL,BE_ACTION_FEATURE_MODE=context-quadrant,BE_RESCUE_FEATURES="${features}",BE_RESCUE_ROLLOUTS="${rollouts}",BE_RESCUE_OUTPUT_DIR="${output_dir}" \
  "${repo_dir}/scripts/slurm_two_stage_rescue_gate.sh"
