...
#SBATCH --job-name=be-screenqa-cal-export
#SBATCH --output=/userhome/cs3/yihangc/Documents/beyond-entropy/slurm-screenqa-calibration-export-%j.out
...
if [[ -n "${tracked_status}" ]]; then
  echo "Tracked worktree must be clean before ScreenQA calibration export" >&2
  exit 2
fi
actual_code_revision=$(git -C "${repo_dir}" rev-parse HEAD)
if [[ "${actual_code_revision}" != "${BE_SCREENQA_EXPECTED_CODE_REVISION}" ]]; then
  echo "ScreenQA calibration export code revision mismatch" >&2
  exit 2
fi
...
if [[ "${actual_candidate_bundle_sha256}" != "${BE_SCREENQA_CANDIDATE_BUNDLE_SHA256}" ]]; then
  echo "ScreenQA candidate bundle SHA-256 mismatch" >&2
  exit 2
fi
if [[ -e "${BE_SCREENQA_CALIBRATION_MANIFEST_DIR}" ]]; then
  echo "ScreenQA calibration manifest output already exists" >&2
  exit 2
fi

export PYTHONPATH="${repo_dir}/src"
cd "${repo_dir}"
"${python_bin}" scripts/export_screenqa_calibration_manifest.py \
  ...
  --formal-output-dir "${BE_SCREENQA_FORMAL_MANIFEST_DIR}" \
  --reserve-output-dir "${BE_SCREENQA_RESERVE_MANIFEST_DIR}" \
  --untouched-output-dir "${BE_SCREENQA_UNTOUCHED_MANIFEST_DIR}" \
  --output-dir "${BE_SCREENQA_CALIBRATION_MANIFEST_DIR}"

cd "${BE_SCREENQA_CALIBRATION_MANIFEST_DIR}"
