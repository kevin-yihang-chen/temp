...
#SBATCH --job-name=be-phase-c-train
#SBATCH --output=/userhome/cs3/yihangc/Documents/beyond-entropy/slurm-phase-c-training-%j.out
...
[[ -z "$(git -C "${repo}" status --porcelain --untracked-files=no)" ]] || {
  echo "tracked worktree must be clean for frozen Phase-C training" >&2; exit 2;
}
cd "${repo}"
...
export PYTHONPATH="${repo}/src" CUBLAS_WORKSPACE_CONFIG=:4096:8
