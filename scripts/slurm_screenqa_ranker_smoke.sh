...
#SBATCH --array=0-1
#SBATCH --job-name=be-screenqa-smoke
#SBATCH --output=/userhome/cs3/yihangc/Documents/beyond-entropy/slurm-screenqa-ranker-smoke-%A-%a.out
...
collect_args=(
  ...
  --scientific-status "ScreenQA ranker-training smoke only; not a benchmark claim"
  --max-new-tokens 32
  --min-pixels 200704
  --max-pixels 602112
...
if [[ "${first_sha}" != "${second_sha}" ]]; then
  echo "ScreenQA resume changed completed rollout bytes" >&2
  exit 2
fi
...
if not passed:
    raise SystemExit("ScreenQA resume audit failed")
