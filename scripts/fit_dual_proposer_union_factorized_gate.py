...
from beyond_entropy.dual_proposer_union_factorized_gate import (
    DUAL_UNION_BOOTSTRAP_RESAMPLES,
    DUAL_UNION_SEED,
    evaluate_dual_proposer_union_factorized_gate,
)
...
def _checked(path: Path, expected: str, name: str) -> Path:
    ...
    if not resolved.is_file() or _sha256(resolved) != expected:
        raise ValueError(f"dual-union {name} SHA-256 mismatch")
    return resolved
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fit the frozen dual-proposer union factorized gate")
    for name in (
    ...
    paths = {
        name: _checked(getattr(args, name), getattr(args, f"expected_{name}_sha256"), name)
        for name in names
    }
    output_dir = args.output_dir.expanduser().resolve()
    if output_dir.exists():
        raise FileExistsError(f"refusing to overwrite dual-union output: {output_dir}")
    ...
    report, score_report, model, score_rows = evaluate_dual_proposer_union_factorized_gate(
        {"docvqa": records},
        ...
        bootstrap_resamples=DUAL_UNION_BOOTSTRAP_RESAMPLES,
        seed=DUAL_UNION_SEED,
    )
