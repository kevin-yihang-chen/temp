...
output_dir="${repo_dir}/artifacts/gate2-chartqa-2500/qwen3b-roi-concise-multiseed"
...
exec sbatch \
  --job-name=be-sem-multiseed2500 \
  --mail-user="${notify_email}" \
  --mail-type=ALL \
  --export=ALL,BE_SEMANTIC_FEATURES="${features}",BE_SEMANTIC_ROLLOUTS="${rollouts}",BE_SEMANTIC_MULTISEED_DIR="${output_dir}" \
  "${repo_dir}/scripts/slurm_semantic_multiseed.sh"
