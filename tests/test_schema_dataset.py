...
from beyond_entropy.dataset import (
    read_jsonl,
    split_by_group,
    validate_sibling_groups,
    write_jsonl,
)
...
def test_image_split_keeps_questions_and_siblings_together():
    ...
    train, test = split_by_group(records, group="image_id", train_fraction=0.6, seed=2)
    assert {record.image_id for record in train}.isdisjoint(
        {record.image_id for record in test}
    )
...
def test_gain_model_features_reject_cost_leakage():
    records = simulate_counterfactual_dataset(n_states=2, num_candidates=2, seed=3)
    zoom = records[1]
    leaked = ActionRecord.from_dict(
        {**zoom.to_dict(), "pre_action_features": {"estimated_action_cost": 0.2}}
    )
    with pytest.raises(ValueError, match="leakage"):
        FeatureEncoder.fit([leaked])


def test_group_requires_answer_sibling_per_replicate():
